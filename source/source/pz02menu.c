...
u8 MenuPuzzle02(void) {
	...
	if (SAVEDATA[1].locked[0]) {
		text_page = 1;
	} else {
		text_page = 0;
	}
	Puzzle02MenuText(0, text_page);
		...
		if (!fade_in && !fade_out && loop) {
			...
			if (TOUCHPAD.held) {
				...
				if (MENU.touch) {
					...
					MENU.id = Puzzle02MenuItemTouch(TOUCHPAD.pos_x, TOUCHPAD.pos_y);
					if (MENU.id != 255) {
						MENU.id_enable = true;
					} else {
				...
				if ((MENU.id != 255) && (MENU.id == Puzzle02MenuItemTouch(MENU.height, MENU.pos)) && MENU.id_enable && !MENU.inertia) {
			...
			if (((text_id != MENU.id) && (MENU.id != 255)) || text_force) {
				...
				if (SAVEDATA[1].locked[text_id]) {
					text_page = 1;
				} else {
					text_page = 0;
				}
				Puzzle02MenuText(text_id, text_page);
			}
...
void LoadPuzzle02MenuFiles(void) {
	...
	// Carga los fondos
	NF_LoadTiledBg("puzz02/bg/menu/p02mbg03", "bg3", 256, 512);
	NF_LoadTiledBg("puzz02/bg/menu/p02mbg02", "bg2", 256, 256);

	// Carga los Sprites
	for (n = 0; n < P02_ITEMS; n ++) {
		sprintf(file, "puzz02/sprite/menu/p02ico%02d", (n + 1));
...
void Puzzle02MenuSprites(u8 id) {
	...
	s16 x = 0;			
	s16 y = 0;
	u8 last = (P02_ITEMS - 1);	// Ultimo ID

	// Calcula el Gfx del Sprite central
	P02MROLLER[2].y = (96 - 48);
	P02MROLLER[2].fy = (P02MROLLER[2].y << 8);
	P02MROLLER[2].gfx = id;
	...
	for (n = 3; n <= 4; n ++) {
		...
		P02MROLLER[n].gfx = gfx;
		// Posicion
		temp += 112;
		P02MROLLER[n].y = temp;
		P02MROLLER[n].fy = (temp << 8);
	}
	...
	for (n = 1; n >= 0; n --) {
		...
		if (temp < 0) {
			value = -temp;
			P02MROLLER[n].fy = (value << 8);
			P02MROLLER[n].fy = -P02MROLLER[n].fy;
		} else {
			P02MROLLER[n].fy = (temp << 8);
		}
	}
	...
	for (n = 0; n < 5; n ++) {
		...
		NF_Vram3dSpriteGfx(P02MROLLER[n].gfx, n, false);
		NF_Vram3dSpritePal(P02MROLLER[n].gfx, n);
		// Desatura las paletas de los puzzles bloqueados
		if (SAVEDATA[1].locked[P02MROLLER[n].gfx]) {
			Desaturate3dSprite(n, 10);
			P02MROLLER[n].bn = true;
		} else {
			P02MROLLER[n].bn = false;
		}
		// Calcula la posicion
		y = P02MROLLER[n].y;
		if (y < -104) y = -104;
...
void Puzzle02MenuMove(s32 move) {
	...
	for (n = 0; n < 5; n ++) {
		...
		P02MROLLER[n].fy += move;
		if (P02MROLLER[n].fy < 0) {
			temp = -P02MROLLER[n].fy;
			P02MROLLER[n].y = (temp >> 8);
			P02MROLLER[n].y = -P02MROLLER[n].y;
		} else {
			P02MROLLER[n].y = (P02MROLLER[n].fy >> 8);
		}
		y = P02MROLLER[n].y;
		if (y < -104) y = -104;
		if (y > 200) y = 200;
		...
		if ((P02MROLLER[n].y >= 384) || (P02MROLLER[n].y <= -288)) id = n;
	}
	...
	if (id != 255) {
		...
		if (P02MROLLER[id].y < 0) {
			temp = -P02MROLLER[id].y;
			P02MROLLER[id].fy = (temp << 8);
			P02MROLLER[id].fy = -P02MROLLER[id].fy;
		} else {
			P02MROLLER[id].fy = (P02MROLLER[id].y << 8);
		}
		// Manda el nuevo GFX a la VRAM y crea el Sprite
		P02MROLLER[id].gfx = gfx;
		NF_Vram3dSpriteGfx(gfx, id, false);
		NF_Vram3dSpritePal(gfx, id);
		// Desatura las paletas de los puzzles bloqueados
		if (SAVEDATA[1].locked[gfx]) { 
			Desaturate3dSprite(id, 10);
			P02MROLLER[id].bn = true;
		} else {
			P02MROLLER[id].bn = false;
		}
		// Calcula la posicion
		y = P02MROLLER[id].y;
		if (y < -104) y = -104;
...
u8 Puzzle02MenuItemTouch(s16 x, s16 y) {
	...
	if ((x >= 64) && (x <= 192) && (y >= 8) && (y <= 184)) {
		...
		for (n = 0; n < 5; n ++) {
			if ((P02MROLLER[n].y > 0) && (y >= P02MROLLER[n].y) && (y <= (P02MROLLER[n].y + 96)) && (P02MROLLER[n].y < 100)) {
				// Guarda el ID
				id = P02MROLLER[n].gfx;
			}
		}
		...
		case 0:		// Pagina 1 (Datos del puzzle)
			...
			switch (GAMEOPTIONS[0].language) {
				...
				case 1:		// Ingles
					...
					sprintf(text, "Played");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 6, text);
					sprintf(text, "%d times", SAVEDATA[1].played[id]);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 7, text);
					sprintf(text, "Resolved");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 9, text);
					sprintf(text, "%d times", SAVEDATA[1].solved[id]);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 10, text);
					sprintf(text, "Unlocked the");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 12, text);
					sprintf(text, "%02d/%02d/%04d", SAVEDATA[1].month[id], SAVEDATA[1].day[id], SAVEDATA[1].year[id]);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 13, text);
					sprintf(text, "The most efficient:");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 15, text);
					sprintf(text, "%d moves", SAVEDATA[1].moves[id]);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 16, text);
		...
		case 1:		// Pagina 2 (Compra el puzzle)
			switch (GAMEOPTIONS[0].language) {
				case 0:		// Castellano
					...
					sprintf(text, "por %d", SAVEDATA[1].cost);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 6, text);
					sprintf(text, "puntos");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 7, text);
					sprintf(text, "En estos momentos");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 8, text);
					sprintf(text, "tienes %d", SAVEDATA[1].credits);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 9, text);
					sprintf(text, "puntos");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 10, text);
				...
				case 1:		// Ingles
					...
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 5, text);
					sprintf(text, "for %d", SAVEDATA[1].cost);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 6, text);
					sprintf(text, "points");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 7, text);
					sprintf(text, "You currently");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 8, text);
					sprintf(text, "have %d", SAVEDATA[1].credits);
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 9, text);
					sprintf(text, "points");
					NF_WriteText16(1, 2, (12 - (strlen(text) >> 1)), 10, text);
...
bool Puzzle02Buy(u8 id) {
	...
	if (SAVEDATA[1].credits >= SAVEDATA[1].cost) {
		// Marcalo como comprado
		SAVEDATA[1].locked[id] = false;
		// Y descuenta el coste
		SAVEDATA[1].credits -= SAVEDATA[1].cost;
		// Incrementa el precio de los demas puzzles
		SAVEDATA[1].cost += 150;
		// Obten la fecha de desbloqueo
		GetCurrentDate(&SAVEDATA[1].day[id], &SAVEDATA[1].month[id], &SAVEDATA[1].year[id]);
		// Marca la compra como realizada
		result = true;
		// Pon el icono en color
		for (n = 0; n < 5; n ++) {
			// Si estas en blanco y negro y estas desbloqueado
			if (P02MROLLER[n].bn && !SAVEDATA[1].locked[P02MROLLER[n].gfx]) {
